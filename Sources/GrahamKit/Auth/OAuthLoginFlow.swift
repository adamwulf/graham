...
public struct OAuthLoginFlow {
    ...
    public func run(credentials: GoogleCredentials, scopes: [GoogleScope]) async throws -> TokenGrant {
        ...
        guard query["state"] == state else {
            throw GrahamError.oauthError("The state parameter in the callback does not match.")
        }
        if let error = query["error"] {
            throw GrahamError.oauthError("Authorization failed: \(error)")
        }
        guard let code = query["code"] else {
            throw GrahamError.oauthError("The callback did not contain an authorization code.")
        }
