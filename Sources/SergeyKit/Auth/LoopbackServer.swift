...
final class LoopbackServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "sergey.loopback")
    ...
    func start() throws -> UInt16 {
        ...
        if semaphore.wait(timeout: .now() + 10) == .timedOut {
            throw SergeyError.oauthError("The local callback server did not start in 10 seconds.")
        }
        if let startError {
            throw SergeyError.oauthError("The local callback server failed: \(startError)")
        }
        guard let port = listener.port?.rawValue else {
            throw SergeyError.oauthError("The local callback server has no port.")
        }
        return port
    }
            ...
            queue.async {
                ...
                self.queue.asyncAfter(deadline: .now() + timeout) {
                    if let waiting = self.callbackContinuation {
                        ...
                        waiting.resume(throwing: SergeyError.oauthError(
